/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { GymServiceWhereInput } from "./GymServiceWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { GymServiceOrderByInput } from "./GymServiceOrderByInput";

@ArgsType()
class GymServiceFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => GymServiceWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => GymServiceWhereInput, { nullable: true })
  @Type(() => GymServiceWhereInput)
  where?: GymServiceWhereInput;

  @ApiProperty({
    required: false,
    type: [GymServiceOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [GymServiceOrderByInput], { nullable: true })
  @Type(() => GymServiceOrderByInput)
  orderBy?: Array<GymServiceOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { GymServiceFindManyArgs as GymServiceFindManyArgs };
